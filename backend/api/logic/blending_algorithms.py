...
def average(image1, image2):
  if image1.shape != image2.shape:
    raise ValueError(
      "Both images must have the same dimensions and number of channels")
  result = ((image1.astype(np.float32) + image2.astype(np.float32)) / 2).astype(
    np.uint8)
  return result
...
def overlay(image1, image2):
  ...
  image1 = image1.astype(np.float32) / 255.0
  image2 = image2.astype(np.float32) / 255.0
  result = np.where(image1 <= 0.5, 2 * image1 * image2,
...
def difference(image1, image2):
  if image1.shape != image2.shape:
    raise ValueError(
      "Both images must have the same dimensions and number of channels")
  result = np.abs(image1.astype(np.int16) - image2.astype(np.int16)).astype(
    np.uint8)
  return result
